		...
		if (SetWaitableTimer(
			...
			while (true) {
				...
				if (CheckMemotyForAccess((HTHANDLE*)ht)) {
					//LOGHT("\nasync snapping waiting timer...",0);
...
#pragma endregion
	...
	HTHANDLE::HTHANDLE(int Capacity, int SecSnapshotInterval, int MaxKeyLength, int MaxPayloadLength, wchar_t FileName[512], LPCWSTR HTUsersGroup) {
		...
		wcscpy((wchar_t*)this->HTUsersGroup, HTUsersGroup);
...
#pragma endregion
	...
	(
		...
		//wcscpy(ht->fileMapName, HTUsersGroup);
		
		//wcscpy((wchar_t*)ht->HTUsersGroup, HTUsersGroup);
	...
	(
		...
		LOGHT("OpenHtFromFile res: ", res);
		if (res != NULL) {
			if (AUTH::checkCurrentUserGroup((LPCWSTR)res->HTUsersGroup)) {
				return res;
			}
			else {
				LOGHT("checkCurrentUserGroup feil: ", 1);
				return NULL;
			}
		}
		else {
			try {
				...
				wofstream myfile;
				myfile.open("d:\\sem6\\os\\os13COM_HT\\lab13\\Release\\authLogw.txt", ios::app);
				myfile << "\nOpenFileMapping:" << mapFileName;
	...
	(
		...
		try {
			...
			LOGHT("CheckMemotyForAccess: true", 1);
			...
			LOGHT("CheckMemotyForAccess: true x2", 1);
			...
			if (creatorPid == GetCurrentProcessId()) {
				...
				if (timer != NULL) {
					if (!CancelWaitableTimer(timer)) {
						//LOGHT("CancelWaitableTimer error ", GetLastError());
						throw "cancel timer";
					}
