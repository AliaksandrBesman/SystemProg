...
#pragma region auxil
	...
	Element* CreateElementOnMemory(HTHANDLE* ht, byte* pointer) {
		...
		result->key = (char*)(pointer + sizeof(Element));
		result->payload = (char*)(pointer + sizeof(Element) + ht->MaxKeyLength);
		return result;
	}
	...
	byte* FindElement(HTHANDLE* ht, char* key) {
		byte* pointer = (byte*)(ht) + sizeof(HTHANDLE);
		int countOfElements = 0;
		Element* tempEl;
		while (countOfElements < ht->Capacity) {
			...
			if (tempEl->keylength != 0 && strcmp(key,(char*)tempEl->key)==0)
				return pointer;
	...
	bool CheckMemotyForAccess(HTHANDLE *hthandle) {
...
#pragma region Element
	...
	Element::Element(const void* key, int keylength) {
		...
		this->key = (void*)key;
		//strcpy_s((char*)this->key, keylength, (const char*)key);
		//((char*)this->key)[keylength] = '\0';
	}
	// for Insert
	Element::Element(const void* key, int keylength, const void* payload, int  payloadlength) {
		...
		//strcpy_s((char*)this->payload, payloadlength, (const char*)payload);
		//((char*)this->payload)[payloadlength] = '\0';
	}
	//TODO
	// for update
...
#pragma endregion
	...
	(
		...
		byte* pointer = FindElement((HTHANDLE*)hthandle, (char*)element->key);
		if (pointer != NULL) {
		...
		if (pointer == NULL) {
			...
			if (element->keylength + 1 > hthandle->MaxKeyLength) {
				((char*)element->key)[hthandle->MaxKeyLength-1] = '\0';
			}
			newElement->keylength = min(element->keylength+1,hthandle->MaxKeyLength);

			if (element->payloadlength + 1 > hthandle->MaxPayloadLength) {
				((char*)element->payload)[hthandle->MaxPayloadLength - 1] = '\0';
			}
			newElement->payloadlength = min(element->payloadlength+1, hthandle->MaxPayloadLength - 1);

			strcpy_s((char*)newElement->key, newElement->keylength, (char*)element->key);

			strcpy_s((char*)newElement->payload, newElement->payloadlength, (char*)element->payload);
	...
	(
		...
		byte* pointer = FindElement((HTHANDLE*)hthandle, (char*)oldelement->key);
		if (pointer == NULL) {
			...
			if (newpayloadlength + 1 > hthandle->MaxPayloadLength) {
				((char*)newpayload)[hthandle->MaxPayloadLength - 1] = '\0';
			}
			int keyLength = ((Element*)pointer)->keylength;
			oldelement = CreateElementOnMemory((HTHANDLE*)hthandle, pointer);
			newpayloadlength = min(newpayloadlength+1, hthandle->MaxPayloadLength);
			//((char*)newpayload)[newpayloadlength - 1] = '\0';
			strcpy_s((char*)oldelement->payload, newpayloadlength, (char*)newpayload);
			((Element * )oldelement)->payloadlength = newpayloadlength;
			((Element*)oldelement)->keylength = keyLength;
			ReleaseMutex(hm);
			return true;
		}
	} 	//  != NULL успешное завершение 
